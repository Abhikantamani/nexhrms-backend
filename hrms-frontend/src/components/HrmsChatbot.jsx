...
export default function HrmsChatbot() {
  ...
  // Auth state
  const [authStep,        setAuthStep]        = useState('login'); // 'login' | 'role' | 'chat'
  const [loggedInUser,    setLoggedInUser]     = useState(null);   // {name, role, emp_id}
  ...
  const handleLogin = (userData) => {
    ...
    // Pre-fill name in convData so bot knows who this is
    const welcomeData = userData.emp_id ? { name: userData.name, emp_id: userData.emp_id } : {};
    setConvData(welcomeData);
    setMessages([{ sender: 'bot', text: ROLE_WELCOME[userData.role](userData.name) }]);
    setAuthStep('chat');
  };

  // ── Skip login → show role/PIN picker ──
  const handleSkipLogin = () => setAuthStep('role');

  // ── Role selected (no login) ──
  const handleRoleSelect = (selectedRole, name) => {
    setRole(selectedRole);
    setUserName(name || '');
    setMessages([{ sender: 'bot', text: ROLE_WELCOME[selectedRole](name) }]);
    setAuthStep('chat');
  };

  // ── New Chat / Logout → back to login ──
  const handleReset = () => {
...
// ── Role-specific welcome messages ─────────────────────────────────
const ROLE_WELCOME = {
  employee: "👋 Hello! I'm your **Personal HR Assistant**.\n\nI can help you with:\n✅ Apply for leave\n📅 Check your leave balance  \n🕐 Mark your attendance\n📄 View your profile\n\nWhat would you like to do today?",
  manager:  "👋 Hello! I'm your **Team Manager Assistant**.\n\nI can help you with:\n✅ Approve or reject leave requests\n👥 View your team's attendance\n📊 See pending leave requests\n📋 View team reports\n\nWhat would you like to do?",
  hr:       "👋 Hello! I'm the **HR Administration Assistant**.\n\nI can help you with:\n➕ Add new employees\n📄 Generate HR letters (Offer, Experience, Relieving)\n👥 View and manage all employees\n📋 Manage leave policies\n\nWhat would you like to do?",
  admin:    "👋 Hello! I'm the **System Administrator**.\n\nI have full access to:\n👥 All employee data\n📊 Full reports and analytics\n🎫 All support tickets\n💰 Salary and payroll info\n\nWhat would you like to do?",
};
...
// Employee has no PIN — anyone can use employee role freely
const ROLE_PINS = {
  employee: null,      // no PIN required
  manager:  '5678',
  hr:       '9012',
  admin:    '0000',
};

// ── Role Selection + PIN Screen ────────────────────────────────────
function RoleSelect({ onSelect }) {
  ...
  const handleRolePick = (role) => {
    if (ROLE_PINS[role] === null) {
      // Employee — no PIN needed, go straight in
      onSelect(role);
      return;
    }
    setSelected(role);
  ...
  return (
    <div className="flex flex-col h-[680px] bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-200">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-4">
        ...
        {/* PIN dots */}
        <div className={`flex gap-4 mb-2 ${shake ? 'animate-[wiggle_0.4s_ease-in-out]' : ''}`}
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-[220px]">
          {KEYPAD.flat().map((key, i) => {
            ...
            return (
              <button key={i}
                onClick={() => key === '⌫' ? handleDelete() : handlePinDigit(key)}
                className={`h-14 rounded-2xl text-lg font-semibold transition active:scale-95 ${
                  key === '⌫'
